extern "C" {
	#include "C:\Users\User\Documents\code\.cpp\PrizmSDK-win-0.6\include\fxcg/display.h"
	#include "C:\Users\User\Documents\code\.cpp\PrizmSDK-win-0.6\include\fxcg/keyboard.h"
	#include "C:\Users\User\Documents\code\.cpp\PrizmSDK-win-0.6\include\fxcg/rtc.h"
}

#include <stdlib.h>
#include <stdint.h>
#include "structs.h"
#include "constants.h"
using namespace std;



namespace utils {


float sqrtApprox(float value) {
	const float EPSILON_LOCAL = 1e-2f; //1-hundreths of precision.
	const int MAX_SQRT_ITERATIONS = 25;
	
	float guess = value * 0.5f;  //x0 = x/2
	int iterations = 0;

	//Iterate until the guess is sufficiently accurate
	while (true) {
		float newGuess = 0.5f * (guess + value / guess);  //Update guess using Newton's method
		iterations++;
		if (abs(newGuess - guess) < EPSILON_LOCAL || iterations >= MAX_SQRT_ITERATIONS) {
			break;  //Stop when the guess is sufficiently accurate
		}
		guess = newGuess;
	}

	return guess;
}


float LENGTH2(vec2 vector) {
	return sqrtApprox((vector.x * vector.x) + (vector.y * vector.y));
}

vec2 NORM2(vec2 vector) {
	float len = LENGTH2(vector);
	return (len<EPSILON) ? vec2(0.0f, 0.0f) : vec2(vector.x/len, vector.y/len);
}


float LENGTH3(vec3 vector) {
	return sqrtApprox((vector.x * vector.x) + (vector.y * vector.y) + (vector.z * vector.z));
}

vec3 NORM3(vec3 vector) {
	float len = LENGTH3(vector);
	return (len<EPSILON) ? vec3(0.0f, 0.0f, 0.0f) : vec3(vector.x/len, vector.y/len, vector.z/len);
}



int RNGIdx = 0;
const unsigned char RNGtbl[256] = {
	
};

int getRNG() {
	RNGIdx = (RNGIdx+1)&0xFF;
	return static_cast<int>(RNGtbl[RNGIdx]);
}

int getRNGAlt() { //Double randomness for further, but slower, randomness.
	RNGIdx = (RNGIdx+1)&0xFF;
	return static_cast<int>(RNGtbl[(RNGtbl[RNGIdx]&0xFF)]);
}

void clearRNG() {
	RNGIdx = 0;
}




int keyResultColumn = 0, keyResultRow = 0, keyResult = 0;
unsigned short keyResultKeycode = 0;

void keyUpdate() {  
	keyResult = GetKeyWait_OS(&keyResultColumn, &keyResultRow, KEYWAIT_HALTOFF_TIMEROFF, 0, 1, &keyResultKeycode);
}
bool isKeyPressed(int basicKeycode) {
	int thisKeyColumn = (basicKeycode & 0xFF00) >> 8;
	int thisKeyRow = basicKeycode & 0x00FF;

	return (keyResult == KEYREP_KEYEVENT) && (thisKeyRow == keyResultRow) && (thisKeyColumn == keyResultColumn);
}




//Only for utils::movement() to use.
Snake shiftSegments(Snake snakeInstance, bool (*snakeMap)[display::GRID_SIZE.x * display::GRID_SIZE.y]) {
	if (snakeInstance.state == S_EATING) {
		snakeInstance.length += 1;
		snakeInstance.state = S_ACTIVE;
	}

	for (int idx = snakeInstance.length - 1; idx > 0; idx--) {
		snakeInstance.segments[idx].x = snakeInstance.segments[idx - 1].x;
		snakeInstance.segments[idx].y = snakeInstance.segments[idx - 1].y;
	}

	return snakeInstance;
}


inline float fmod(float numerator, float divisor) {
	if (divisor == 0.0f) return 0.0f;
	int divis = static_cast<int>(numerator / divisor);
	return numerator - (divisor * divis);
}


Snake createSnake(vec2 startPoint) {
	Snake snakeInstance = Snake();
	snakeInstance.segments[0] = startPoint;
	snakeInstance.length = 1;
	snakeInstance.state = S_ACTIVE; //Active snake.
	return snakeInstance;
}

Snake movement(Snake snakeInstance, bool (*snakeMap)[display::GRID_SIZE.x * display::GRID_SIZE.y], int direction, vec2 applePos) {
	vec2 headPos = snakeInstance.segments[0];
	snakeInstance = shiftSegments(snakeInstance, snakeMap);
	if (snakeInstance.length == (display::GRID_SIZE.x * display::GRID_SIZE.y)) {
		snakeInstance.state = S_WIN; //You win :)
		return snakeInstance;
	}
	vec2 tailPos = snakeInstance.segments[snakeInstance.length-1];

	switch (direction) {
		case D_UP:
			headPos.y -= 1;
			break;
		case D_DOWN:
			headPos.y += 1;
			break;
		case D_LEFT:
			headPos.x -= 1;
			break;
		case D_RIGHT:
			headPos.x += 1;
			break;
	}


	//Allow moving off the edges to the other side
	headPos.x = fmod((headPos.x + display::GRID_SIZE.x), display::GRID_SIZE.x);
	headPos.y = fmod((headPos.y + display::GRID_SIZE.y), display::GRID_SIZE.y);


	(*snakeMap)[(headPos.y * display::GRID_SIZE.x) + headPos.x] = true;
	(*snakeMap)[(tailPos.y * display::GRID_SIZE.x) + tailPos.x] = false;


	for (int idx = 1; idx < snakeInstance.length; idx++) {
		vec2 thisSegmentPos = snakeInstance.segments[idx];
		if (headPos.x == thisSegmentPos.x && headPos.y == thisSegmentPos.y) {
			snakeInstance.state = S_LOSE; //You crashed :(
			return snakeInstance;
		}
	}


	snakeInstance.segments[0] = headPos;
	if (headPos.x == applePos.x && headPos.y == applePos.y && snakeInstance.state != S_EATING) {
		snakeInstance.length += 1;
		snakeInstance.state = S_EATING; //Eating

		vec2 prevLast = snakeInstance.segments[snakeInstance.length-2];
		snakeInstance.segments[snakeInstance.length-1] = prevLast;
		snakeInstance.segments[snakeInstance.length] = prevLast;
		return snakeInstance;
	}

	return snakeInstance;
}


int randomInRange(int min, int max) {
	return min + rand() % (max - min + 1);
}



}