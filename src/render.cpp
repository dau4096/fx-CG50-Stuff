...
void drawSnake(Snake snakeInstance, int direction) {
	...
	if (snakeInstance.length <= 1) {
		...
		if (headDir == INVALID) return; //HeadDir is invalid somehow.
	}
	...
	for (int idx = 1; idx < snakeInstance.length-1; idx++) {
		...
		if (startDir == INVALID || endDir == INVALID) continue; //Overlapping somehow, should never happen. Proceed to next segment.
	...
	if (tailDir == INVALID) return; //Invalid in some way, should never happen. Just exit if so.
